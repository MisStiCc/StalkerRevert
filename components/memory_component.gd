...
var vision_range: float = 30.0:
    set(value):
        ...
        Logger.debug("Дальность зрения изменена на " + str(vision_range), "MemoryComponent")
...
func _ready():
    set_process(true)
    Logger.debug("MemoryComponent инициализирован с дальностью " + str(vision_range), "MemoryComponent")
...
func _clean_invalid_objects(current_time: float):
    ...
    for a in known_anomalies:
        if not is_instance_valid(a):
            to_remove.append(a)
        elif current_time - object_timers.get(a.get_instance_id(), 0) > memory_duration:
            to_remove.append(a)
            threat_lost.emit(a)
    ...
    for m in known_mutants:
        if not is_instance_valid(m):
            to_remove.append(m)
        elif current_time - object_timers.get(m.get_instance_id(), 0) > memory_duration:
            to_remove.append(m)
            threat_lost.emit(m)
    ...
    for a in known_artifacts:
        if not is_instance_valid(a):
            ...
            # Проверяем, не собран ли артефакт
    ...
    for s in known_stalkers:
        if not is_instance_valid(s) or s == stalker:
            to_remove.append(s)
        elif current_time - object_timers.get(s.get_instance_id(), 0) > memory_duration:
            to_remove.append(s)
...
func _scan_for_anomalies(tree: SceneTree, pos: Vector3, current_time: float):
    ...
    for a in anomalies:
        ...
        if dist <= vision_range:
            ...
            if not id in object_timers:
                known_anomalies.append(a)
                threat_detected.emit(a, "anomaly")
                Logger.debug("Обнаружена аномалия на расстоянии " + str(dist), "MemoryComponent")
    ...
    for m in mutants:
        ...
        if dist <= vision_range:
            ...
            if not id in object_timers:
                ...
                Logger.debug("Обнаружен мутант на расстоянии " + str(dist), "MemoryComponent")
            object_timers[id] = current_time
    ...
    for a in artifacts:
        ...
        if dist <= vision_range:
            ...
            if not id in object_timers:
                ...
                Logger.debug("Обнаружен артефакт на расстоянии " + str(dist), "MemoryComponent")
            object_timers[id] = current_time
    ...
    for s in stalkers:
        ...
        if dist <= vision_range:
            ...
            if not id in object_timers:
                ...
                Logger.debug("Обнаружен другой сталкер на расстоянии " + str(dist), "MemoryComponent")
            object_timers[id] = current_time
...
func is_known(node: Node) -> bool:
    ...
    return id in object_timers


func get_memory_age(node: Node) -> float:
    var id = node.get_instance_id()
    if id in object_timers:
...
func clear_memory():
    ...
    object_timers.clear()
    memory_cleared.emit()
    Logger.debug("Память очищена", "MemoryComponent")
