...
func _ready():
    ...
    Logger.info("BaseStalker инициализирован: тип=" + str(stalker_type) + " поведение=" + str(behavior_type), "BaseStalker")
...
func _apply_type_parameters():
    ...
    Logger.debug("Параметры типа применены: " + str(stalker_type), "BaseStalker")
...
func _on_threat_detected(threat: Node, type: String):
    if not current_target:
        ...
        Logger.debug("Цель обнаружена: " + str(threat) + " тип: " + type, "BaseStalker")
...
func _on_threat_lost(threat: Node):
    if current_target == threat:
        ...
        Logger.debug("Цель потеряна: " + str(threat), "BaseStalker")
    ...
    if carry_component and not carry_component.has_artifact():
        if carry_component.can_pick_up(artifact):
            ...
            if navigation_component:
                ...
                Logger.debug("Движение к артефакту", "BaseStalker")
...
func _on_artifact_picked_up(artifact: Node):
    ...
    Logger.info("Артефакт поднят, ценность: " + str(value), "BaseStalker")


func _on_artifact_dropped(artifact: Node):
    artifact_dropped.emit(artifact)
    Logger.info("Артефакт выброшен", "BaseStalker")


func _on_artifact_stolen(artifact: Node):
    var value = artifact.get_value() if artifact.has_method("get_value") else 0
    artifact_stolen.emit(artifact, value)
    Logger.info("Артефакт украден, ценность: " + str(value), "BaseStalker")
...
func _try_attack():
    ...
    if dist <= attack_range:
        ...
        if randf() < critical_chance:
            final_damage *= 2.0
            Logger.debug("Критический удар!", "BaseStalker")
...
func set_behavior(behavior: GameEnums.StalkerBehavior):
    ...
    Logger.info("Поведение изменено на: " + str(behavior), "BaseStalker")
