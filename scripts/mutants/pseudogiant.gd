...
## Псевдогигант - огромный монстр
## Бьёт по земле, создавая ударные волны

@export var ground_stomp_force: float = 30.0  # Сила удара по земле
@export var shockwave_radius: float = 10.0  # Радиус ударной волны
@export var shockwave_damage: float = 40.0  # Урон от волны
@export var stomp_cooldown: float = 5.0  # Кулдаун удара
...
func _ready():
	# Псевдогигант - огромный и медленный, но очень сильный
	health = 400.0
	max_health = 400.0
	speed = 2.5  # Медленный
	damage = 45.0  # Огромный урон
	armor = 40.0  # Огромная броня
	biomass_cost = 250.0  # Очень дорогой
	detection_radius = 30.0  # Видит далеко
...
func _physics_process(delta):
	...
	# Если топчем - не двигаемся
	if is_stomping:
		velocity = Vector3.ZERO
		move_and_slide()
		return
	
	# Проверяем, можно ли ударить
...
func _chase(delta):
	"""Медленное, но неумолимое преследование"""
...
func _ground_stomp():
	"""Удар по земле - создание ударной волны"""
...
func _create_shockwave():
	...
	# Наносим урон всем сталкерам в радиусе
...
func _create_stomp_visuals():
	...
	shockwave.global_position.y = 0.1  # Чуть выше земли
...
func _attack(delta):
	...
	if attack_timer.is_stopped():
		...
		# С небольшим шансом - сбиваем с ног
...
func take_damage(dmg: float):
	# Псевдогигант имеет очень толстую шкуру
	# Урон уменьшен на 50%
	super.take_damage(dmg * 0.5)
