extends BaseMutant
class_name ZombieMutant

## Зомби - потерявшие рассудок из-за Выжигателя Мозгов
## Медленные, но опасны в больших группах

@export var rage_threshold: float = 50.0  # Порог ярости (полученный урон)
@export var grab_damage: float = 15.0  # Урон от захвата
@export var infection_chance: float = 0.2  # Шанс заражения

var accumulated_rage: float = 0.0  # Накопленная ярость
var is_raging: bool = false  # Режим ярости

func _ready():
	# Зомби - медленные, но выносливые
	health = 180.0
	max_health = 180.0
	speed = 2.0  # Очень медленные
	damage = 10.0
	armor = 15.0
	biomass_cost = 30.0  # Дешёвые
	mutant_type = "zombie"
	
	super._ready()
	
	# По умолчанию ищут сталкера
	current_state = State.PATROL
	
	print("Zombie mutant initialized")


func _physics_process(delta):
	if current_state == State.DEAD:
		return
	
	# Проверяем накопление ярости
	if accumulated_rage > rage_threshold and not is_raging:
		_become_raging()
	
	# Если в ярости - двигаемся быстрее
	if is_raging:
		speed = 4.0  # Ускорение в ярости
	else:
		speed = 2.0
	
	super._physics_process(delta)


func _patrol(delta):
	# Зомби просто бредут к сталкерам
	var stalkers = get_tree().get_nodes_in_group("stalkers")
	var nearest_stalker = null
	var nearest_dist = INF
	
	for stalker in stalkers:
		if is_instance_valid(stalker):
			var dist = global_position.distance_to(stalker.global_position)
			if dist < detection_radius and dist < nearest_dist:
				nearest_dist = dist
				nearest_stalker = stalker
	
	if nearest_stalker:
		target_stalker = nearest_stalker
		current_state = State.CHASE
	
	super._patrol(delta)


func _chase(delta):
	"""Медленное преследование"""
	if not target_stalker or not is_instance_valid(target_stalker):
		current_state = State.PATROL
		return
	
	var direction = (target_stalker.global_position - global_position).normalized()
	velocity = direction * speed
	
	# Если достаточно близко - атакуем
	if global_position.distance_to(target_stalker.global_position) < 1.5:
		current_state = State.ATTACK


func _attack(delta):
	"""Атака - захват и укус"""
	if not target_stalker or not is_instance_valid(target_stalker):
		current_state = State.PATROL
		return
	
	var dist = global_position.distance_to(target_stalker.global_position)
	if dist > 2.0:
		current_state = State.CHASE
		return
	
	# Захват (останавливает сталкера)
	if attack_timer.is_stopped():
		target_stalker.take_damage(damage)
		attacked_stalker.emit(target_stalker)
		
		# Попытка заражения
		if randf() < infection_chance:
			_try_infect(target_stalker)
		
		attack_timer.start()


func _try_infect(stalker: Node3D):
	"""Попытка заражения сталкера"""
	if stalker.has_method("apply_infection"):
		stalker.apply_infection(10.0)  # 10 секунд заражения
		print("Zombie: заразил сталкера!")


func _become_raging():
	"""Переход в режим ярости"""
	print("Zombie: вхожу в ярость!")
	is_raging = true
	accumulated_rage = 0.0
	
	# Увеличиваем урон в ярости
	damage = 25.0
	
	# Можно добавить визуальный эффект (покраснение и т.д.)


func take_damage(dmg: float):
	# Накапливаем ярость от полученного урона
	accumulated_rage += dmg
	
	# Зомби имеют регенерацию здоровья
	if health < max_health:
		health += dmg * 0.1  # 10% полученного урона восстанавливается
	
	super.take_damage(dmg)
