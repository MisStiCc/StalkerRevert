...
## Хранит прогресс игрока между забегами
...
func get_bonus(upgrade_type: String) -> float:
	...
	# Для некоторых бонусов это абсолютное значение, для других - процент
...
func get_all_artifacts() -> Dictionary:
	return {
		"common": {"count": get_artifact_count("common"), "value": get_total_artifact_value("common"), "items": artifacts_common},
		"rare": {"count": get_artifact_count("rare"), "value": get_total_artifact_value("rare"), "items": artifacts_rare},
		"legendary": {"count": get_artifact_count("legendary"), "value": get_total_artifact_value("legendary"), "items": artifacts_legendary}
	}
